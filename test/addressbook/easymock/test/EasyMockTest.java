...
public class EasyMockTest {
	...
	/*
	 * private PreparedStatement saveStatement; private PreparedStatement
	 * updateStatement; private PreparedStatement getStatement; private
	...
	public void testConnect_returnsTrue() throws SQLException {
		...
		EasyMock.expect(connection.prepareStatement(strSaveAddress, 1))
				.andReturn(statement);
		EasyMock.expect(connection.prepareStatement(strUpdateAddress))
				.andReturn(statement);
		EasyMock.expect(connection.prepareStatement(strGetAddress)).andReturn(
				statement);
		EasyMock.expect(connection.prepareStatement(strDeleteAddress))
				.andReturn(statement);
	...
	@Test
	public void testConnect_SQLExceptionCaughtAndReturnsFalse()
			throws SQLException {
		EasyMock.expect(dbaccess.connect()).andReturn(true);
		EasyMock.expect(dbaccess.getConnection()).andReturn(connection);
		EasyMock.expect(connection.prepareStatement(strSaveAddress, 1))
	...
	// EasyMock.expectLastCall().times(11);
	//
	// EasyMock.expect(statement.executeUpdate()).andThrow(new
	// SQLException(""));
	//
	// EasyMock.replay(dbaccess, connection, statement, results);
	//
	...
	public void testGetAddress() throws SQLException {
		...
		EasyMock.expect(results.getString(anyString())).andReturn("test")
				.times(11);
		EasyMock.expect(results.getInt(anyString())).andReturn(id);

		Address Expectedaddress = new Address("test", "test", "test", "test",
				"test", "test", "test", "test", "test", "test", "test", id);
